...
- 双角色权限：访客仅看概览 + 管理员完整控制
- 中英文切换

## 快速开始

### Docker 部署（推荐，私有代理池场景）
...
> 当前 `docker-compose.yml` 默认将 `7776-7780 / 7778` 仅绑定到 `127.0.0.1`，更适合作为**云服务器私有代理池**使用。

私有访问建议：

```bash
# 通过 SSH 隧道把云服务器本地端口转发到自己的电脑
ssh -L 7776:127.0.0.1:7776 \
    -L 7777:127.0.0.1:7777 \
    -L 7778:127.0.0.1:7778 \
    -L 7779:127.0.0.1:7779 \
    -L 7780:127.0.0.1:7780 \
    user@your-server
```

也可以使用 WireGuard / Tailscale 等私网方式访问，不建议直接暴露这些端口到公网。
...
为降低 SSRF 与滥用风险，**匿名贡献订阅入口默认已关闭**。
订阅 URL 还会做基础安全校验：仅允许 `http/https`，并拒绝本地回环、私网、链路本地与云元数据等地址。
...
| 场景 | 建议 |
|------|------|
| 私有部署（推荐） | 端口仅绑定 `127.0.0.1`，启用代理认证，通过 SSH 隧道 / WireGuard / Tailscale 使用 |
| 公网部署 | **不推荐直接裸暴露**；至少需代理认证、强密码、外层防火墙/反代访问控制 |
| 本地测试 | 可直接使用默认 localhost 端口 |

## 环境变量

| 变量 | 默认值 | 必须 | 说明 |
|------|--------|------|------|
| `WEBUI_PASSWORD` | `goproxy` | 是 | WebUI 登录密码；建议在 `.env` 中改为强密码 |
| `PROXY_AUTH_ENABLED` | `false` | 否 | 代理认证开关；私有部署也建议启用 |
...
- 管理员导入的订阅内容由使用者自行负责，项目维护者不对其内容承担任何责任
